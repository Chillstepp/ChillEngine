...
namespace ChillEngine::graphics::d3d12::content
{
    namespace
    {
        ...
        id::id_type create_root_signature(material_type::type type, shader_flags::flags flags)
        {
            ...
            auto pair = mtl_rootsig_map.find(key);
            if (pair != mtl_rootsig_map.end())
            {
    ...
    namespace render_item 
    {
        ...
        //buffer[1 ... n] = d3d12_render_item_ids(n is the number of love-level render item ids ,it is also equal to the number of submeshes/materials ids)
        ...
        void get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
        {
            ...
            ChillEngine::content::get_lod_offset(frame_cache.geometry_ids.data(), info.thresholds, count, frame_cache.lod_offsets);
