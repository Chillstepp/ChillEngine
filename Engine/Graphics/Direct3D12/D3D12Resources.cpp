...
namespace ChillEngine::graphics::d3d12
{
    ...
    void descriptor_heap::free(descriptor_handle& handle)
    {
        ...
        const u32 index = (handle.cpu.ptr - _cpu_start.ptr)/_descriptor_size;
        assert(handle.index == index);

        //safely release resource, some resource may still be referenced by gpu
        const u32 frane_index = core::current_frame_index();
        _deferred_free_indices[frane_index].push_back(index);
...
/*<-------------------------------------------------------d3d12_texture------------------------------------------------------------------->*/
    d3d12_texture::d3d12_texture(d3d12_texture_init_info info)
    {
        ...
        else if(info.heap && info.desc)
        {
            ...
            device->CreatePlacedResource(info.heap, info.allocation_info.Offset, info.desc, info.initial_state, clear_value, IID_PPV_ARGS(&_resource));
        }
        else if(info.desc)
        {
            assert(!info.resource);
            device->CreateCommittedResource(&d3dx::heap_properties.default_heap, D3D12_HEAP_FLAG_NONE, info.desc, info.initial_state, clear_value, IID_PPV_ARGS(&_resource));
        }
